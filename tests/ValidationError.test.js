...
describe('ValidationError', () => {
  ...
  it('throws correct NOT_JSONABLE errors without path', () => {
    let error = new ValidationError({ errorType: 'NOT_JSONABLE' })
    expect(error.message).toBe('Input is not JSONABLE')
  });

  it('throws correct NOT_JSONABLE errors with path', () => {
    let error = new ValidationError({ errorType: 'NOT_JSONABLE', path: '.data'})
    expect(error.message).toBe('Input is not JSONABLE: (path: .data)')
  });
